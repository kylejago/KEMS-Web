...
function privateHostname(value = "") {
  ...
  if (host === "localhost" || host === os.hostname().toLowerCase() || host === `${os.hostname().toLowerCase()}.local` || host.endsWith(".local")) return true;
  return net.isIP(host) ? privateAddress(host) : false;
}
...
function requestIsLocal(request) {
  return privateAddress(request.socket.remoteAddress || "");
}
...
function extractToken(input) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    const match = INSTALL_LINE.exec(line);
...
function status() {
  ...
  const connected = state === "active" && logs.some((line) => /registered tunnel connection|connection .* registered|serving tunnel/i.test(line));
  return {
    available: true,
    localOnly: true,
    busy,
...
function architectureAsset() {
  ...
  if (!assets[architecture]) throw new Error(`Unsupported Pi architecture for automatic cloudflared installation: ${architecture || os.arch()}.`);
...
export function startRemoteAccessServer() {
  const server = http.createServer(async (request, response) => {
    if (!requestIsLocal(request)) return sendJson(response, 403, { error: "KEMS remote-access setup is available only from the local network." });
    const origin = localOrigin(request);
    if (request.method === "OPTIONS") {
      if (!origin) return sendJson(response, 403, { error: "Local KEMS origin required." });
    ...
    try {
      if (url.pathname === "/health" && request.method === "GET") return sendJson(response, 200, { ok: true, localOnly: true }, origin);
      if (!origin) return sendJson(response, 403, { error: "Open Remote Access Setup from the local KEMS Pi website." });
      if (url.pathname === "/status" && request.method === "GET") return sendJson(response, 200, status(), origin);
      if (url.pathname === "/install" && request.method === "POST") {
        const body = await readBody(request);
        const result = await install(body.command || body.token || "");
        return sendJson(response, 200, result, origin);
      }
      if (url.pathname === "/action" && request.method === "POST") {
        const body = await readBody(request);
        return sendJson(response, 200, action(String(body.action || "")), origin);
      }
      return sendJson(response, 404, { error: "Not found." }, origin);
    } catch (error) {
      return sendJson(response, 400, { error: redact(error.message) }, origin);
    }
  });

  server.listen(PORT, HOST, () => {
    console.log(`KEMS remote-access setup listening on LAN port ${PORT}; Cloudflare origin remains http://localhost:${WEB_PORT}`);
  });
  return server;
}
