...
KEMS Web is the read-only property companion, installable mobile PWA and public-site source for **KEMS**. Home Assistant/KEMS remains the source of truth for each property.

Release versions identify published repository states rather than the product or implementation name. The current maintenance release metadata is `0.8.0-alpha8-web.1`; live UI, modules and cache identities remain release-independent.
...
The accepted authenticated-PWA manifest path is preserved. Every property page loads `site.webmanifest` with `crossorigin="use-credentials"`, so the browser includes the Cloudflare Access session when it performs its own manifest request.
...
The separate Flutter `KEMS-Android` 1.4.1 app remains a frozen legacy implementation while the KEMS Web PWA completes real-device acceptance. New dashboard work belongs in KEMS Web so Live, Compare and Full KEMS Agile are maintained once. A future Play Store package, if required, should be a thin wrapper around the verified PWA rather than a second dashboard implementation.
...
The suite checks syntax, brand identity, public-demo isolation and delay, Cloudflare-login boundaries, fixture compatibility, historical Agile evidence, comparison/ROI, Pi deployment behaviour, mobile shell/install-state/Cloudflare manifest-credential contracts, product-identity separation and the actual runtime `/site.webmanifest` served by the Pi stack.
