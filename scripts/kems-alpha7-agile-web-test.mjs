...
if (pkg.version !== "0.7.0-alpha7-web.15") throw new Error(`Unexpected package version ${pkg.version}`);
...
if (!pkg.scripts.test.includes("web15-remote-access-test.mjs")) throw new Error("Web.15 remote-access regression is not in npm test");
...
if (!serviceWorker.includes("kems-alpha7-web15-shell-v1") || !serviceWorker.includes("/products.html") || !serviceWorker.includes("/agile.html") || !serviceWorker.includes("/remote-access.html")) throw new Error("PWA cache is not Web.15 remote-access aware");
...
console.log("KEMS Web.15 contract passed: versions aligned, four products + Full KEMS Agile read-only parity + local remote-access setup present, public boundary preserved.");
