...
mustContain("deploy/remote-access-service.mjs", ["const HOST = \"127.0.0.1\"", "4175", "cloudflared", "--token-file", "http://localhost:4173", "mode: 0o600", "0.7.0-alpha7-web.17"]);
mustContain("install.sh", ["gateway.mjs", "kems-web-manager.service", "kems-web-bundle-agent.service", "kems-web-remote-access.service", "remote-access-service.mjs", "/var/lib/kems-web-management", "http://127.0.0.1:4175/health", "Remote Access setup helper did not pass its loopback health check"]);
...
mustContain("public/remote-access.html", ["Remote access", "Cloudflare Tunnel connector", "http://localhost:4173", "kyle.kems.uk", "Install &amp; connect", "brand-lockup.svg?v=alpha7web17", "page-brand-lockup"]);
mustContain("public/remote-access.js", ["/api/remote-access", "\"/install\"", "\"/action\""]);

console.log("Pi deployment checks passed for Web.17 bootstrap repair, gateway and loopback Remote Access helper.");
