...
const mustMatch = (file, pattern, message) => {
  const text = fs.readFileSync(file, "utf8");
  if (!pattern.test(text)) throw new Error(`${file} missing: ${message}`);
};
...
mustMatch("deploy/remote-access-service.mjs", /HELPER_VERSION = "0\.7\.0-alpha7-web\.(?:19|[2-9]\d+)"/, "Web.19+ Remote Access helper baseline");
