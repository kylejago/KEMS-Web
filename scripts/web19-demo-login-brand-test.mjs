...
const hasWeb19Baseline = alphaNumber > 7 || webNumber >= 19;
const hasWeb30Evidence = alphaNumber > 7 || webNumber >= 30;

expect(hasWeb19Baseline, "package.json must retain the Web.19+ baseline");
...
expect(!sync.includes("kems_full_brand_concept.png") && !sync.includes("67ad8c3e"), "current release must not fetch old PNG brand");
...
if (hasWeb30Evidence) {
  for (const marker of [
    ...
    "evKwh",
    "systemCostGbp"
  ]) expect(gateway.includes(marker), `current public evidence publisher missing ${marker}`);
  ...
  expect(gateway.includes("no live power, EV state/SOC"), "public privacy boundary must explicitly exclude live EV/property telemetry");
} else {
...
if (hasWeb30Evidence) {
  expect(demo.includes("let period = 'day'"), "public comparison must default to the latest delayed day");
  expect(demo.includes("economicResult"), "public demo must distinguish profit from cost");
  expect(demo.includes("evKwh"), "public demo must display delayed aggregate EV evidence");
  expect(demo.includes("systemCostGbp"), "public ROI must use configured system-cost evidence");
  expect(demo.includes("completeCompareDay"), "comparison periods must enable only when all four products are comparable");
  expect(demo.includes("kems-panel-stage"), "public Agile page must retain the Pi-style panel graphic");
}
...
expect(worker.includes(`kems-alpha${alphaNumber}-web${webNumber}-shell-v1`), "current Web release must advance PWA cache");
...
for (const file of publicPages) {
  const text = read(file);
  expect(/alpha(?:7web(?:19|20|21)|8web0)/.test(text), `${file} must retain a supported public-site cache version`);
  expect(!text.includes("alpha7web18"), `${file} still references Web.18 assets`);
}
...
expect(helper.includes(`const HELPER_VERSION = "${pkg.version}"`), "Remote Access helper must align with current Web/Pi release identity");
console.log(`${pkg.version} exact SVG, delayed demo and Cloudflare login contract passed.`);
