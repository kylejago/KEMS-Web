  ...
  if (url.pathname.startsWith("/api/history/period/")) {
    ...
    const result = ids.map((id) => {
      ...
      return Array.from({ length: 42 }, (_, index) => {
        const value = Number.isFinite(base) ? base * (0.9 + ((index % 9) / 100)) : source.state;
...
try {
  ...
  if (!js.includes("Awaiting export tariff") || !js.includes("solarBatteryCurve") || !js.includes("Alpha5 period ledger")) throw new Error("Alpha5 frontend features missing.");
  ...
  console.log(`KEMS alpha5 fixture passed: ${tested.kemsEntityCount} entities, native Day/Week/Month/Year/All-time totals and no-export policy verified.`);
} finally {
  child.kill("SIGTERM");
