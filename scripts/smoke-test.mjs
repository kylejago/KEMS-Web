...
try {
  ...
  const [health, config, setup, site, manifest, live, history, html, js, agileHtml, agileJs, css, system, scenarios] = await Promise.all([
  ...
  if (!html.includes("Alpha7 energy dashboard")) throw new Error("HTML shell incomplete.");
  ...
  if (!agileHtml.includes("Agile Smart Export") || !agileJs.includes("sensor.kems_agile_shadow_status")) throw new Error("Alpha7 Agile frontend incomplete.");
  ...
  console.log(`Smoke test passed: ${packageVersion}, setup ready, ${config.mappedEntityCount} mappings, Alpha7 Agile shell present.`);
} finally {
  child.kill("SIGTERM");
