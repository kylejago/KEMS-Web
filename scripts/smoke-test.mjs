...
try {
  ...
  if (!html.includes("four-product energy dashboard") || !html.includes("Products") || !html.includes("Cost &amp; ROI")) throw new Error("Web.14 HTML shell incomplete.");
  if (!productsHtml.includes("Four clear product levels") || !productModel.includes('label: "Full KEMS Agile"')) throw new Error("Web.14 product model/page incomplete.");
  ...
  console.log(`Smoke test passed: ${packageVersion}, setup ready, ${config.mappedEntityCount} mappings, four-product Alpha7 shell present.`);
} finally {
  child.kill("SIGTERM");
