...
const port=4197;
const dataDir=fs.mkdtempSync(path.join(os.tmpdir(),"kems-alpha6-smoke-"));
...
try{
  ...
  const [health,config,setup,site,manifest,live,history,html,js,css,system,scenarios]=await Promise.all([
  ...
  if(!health.ok || health.version!=="0.7.0-alpha6-web.7")throw new Error("Health/version failed.");
  ...
  if(!html.includes("alpha6 energy dashboard"))throw new Error("HTML shell incomplete.");
  ...
  console.log(`Smoke test passed: setup ready, ${config.mappedEntityCount} alpha6 mappings.`);
