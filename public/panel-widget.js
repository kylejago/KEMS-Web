...
function batteryNode(v) {
  if (!Number.isFinite(v.batterySoc)) {
    return `<div class="kems-web-node battery unavailable"><div class="kems-web-icon">▰</div><span>BATTERY</span><strong>—</strong><small>Physical battery unavailable</small></div>`;
  }
  ...
  const low = soc < 10 ? " low" : "";
  return `<div class="kems-web-node battery${low}"><div class="kems-battery-gauge" aria-hidden="true"><i style="--soc:${soc}%"></i></div><span>BATTERY</span><strong>${pct(soc)}</strong><small>${state}${Number.isFinite(v.battery) ? ` · ${kw(Math.abs(v.battery))}` : ""}</small></div>`;
}
...
function ensurePanel() {
  ...
  if (!panel) {
    shell.innerHTML = `<div class="kems-web-panel" role="img" aria-label="KEMS live energy panel"><div class="kems-web-status" data-panel-status></div><div class="kems-web-stage"><div class="kems-web-connector solar-link"><i></i></div><div class="kems-web-connector grid-link"><i></i></div><div class="kems-web-connector battery-link"><i></i></div><div class="kems-web-connector ev-link"><i></i></div><div class="kems-web-node solar" data-node="solar"></div><div class="kems-web-node grid" data-node="grid"></div><div class="kems-web-node home" data-node="home"></div><div data-node="battery"></div><div class="kems-web-node ev" data-node="ev"></div></div></div>`;
...
function render(data) {
  ...
  panel.querySelector('[data-node="battery"]').innerHTML = batteryNode(v);
